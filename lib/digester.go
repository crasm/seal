...
// A Digester knows how to read and write entire seal files in a
// single pass.
type Digester interface {
	...
	Wrap(io.Reader, io.WriteSeeker) (*Seal, error)
	// Wrap the contents of the io.Reader, but buffer the output to a
	// temporary file while digesting the input.
	WrapBuffered(io.Reader, io.Writer) (*Seal, error)
	Unwrap(io.Reader, io.Writer) (*UnwrappedSeal, error)
}
